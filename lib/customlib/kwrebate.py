...
def rebate_refresh_py(rebate_id, cookie=init_cookie):
    '''
    更新满减活动信息
    :param query_str:
    :param cookie:
    ...
    '''
    url = '%s/api/admin/discount-rebate/%s' % (home_page_url, rebate_id)
    return do_put(url, {}, cookie=cookie)
...
def rebate_delete_py(rebate_id, cookie=init_cookie):
    '''
    删除满减活动
    :param query_str:
    :param cookie:
...
def get_select_product_list_py(conf={}, cookie=init_cookie):
    ...
    :return: list
    '''
    rebate_data = copy.deepcopy(rebate_select_product_data)
    data = rebate_selected_info_py(rebate_data)
