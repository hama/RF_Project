...
001 Test Forward To Shipping Page
    ...
    [Tags]    P0
    log    测试页面基本内容
    Page Should Contain    ${contentB_tax_head_country}
    Page Should Contain    ${contentB_tax_head_taxPrice}
    Page Should Contain    ${contentB_tax_head_location}
...
002 Test Tax Page Normal
    ...
    Mouse Over    ${locatorB_tax_help}
    Sleep    1
    Page Should Contain    ${contentB_tax_tips_count}
    Log    测试: 鼠标移动到设置按钮上
    Mouse Over    ${locatorB_tax_setting}
    Wait Until Page Contains    ${contentB_tax_tips_setting}

003 Test Tax Price Setting
    [Documentation]    测试: 税费设置
    [Tags]    P0
    Wait And Click Element    ${locatorB_tax_setting}
    ...
    log    检查: 税费设置是否生效
    Click Element    ${locatorB_tax_setting}
    Wait Until Page Contains    保 存
    Sleep    2
    Check City Data
    Click Element    ${locatorB_tax_close}
...
004 Test Toggle Tax Switch
    ...
    ${dataLength}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list.length;
    log    列表应该不为空，有数据存在
    Should Be True    ${dataLength}>=1
    # 获取原始开关值
    ${rawSwitch}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[0].is_enable;
    Wait And Click Element    id:switch_1
    Page Should Contain    ${contentB_tax_tips_setting}
    Check response status
    # 获取设置后开关值
    ${newSwitch}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[0].is_enable;
...
Check Tax Page List
    ...
    \    ${country_name}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[${index}].country_name;
    \    ${country_name_en}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[${index}].country_name_en;
    \    ${country_price}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[${index}].country_price.toString();
    \    ${zone_total}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[${index}].zone_total.toString();
    \    ${zone_total_msg}=    Set Variable If    '${zone_total}'=='0'    ${contentB_tax_data_all}    ${zone_total}
...
Set Random Price
    ...
    ${zone_count}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[0].zone_list.length;
...
Check City Data
    ...
    \    ${rowIndex}=    Evaluate    int(${index}) + 2
    \    ${inputIndex}=    Evaluate    int(${index}) + 1
    \    ${zone_name}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[0].zone_list[${index}].zone_name;
    \    ${zone_price}=    Execute JavaScript    return responseMap.get("${apiB_tax_list}").data.list[0].zone_list[${index}].price.toString();
...
Check response status
    ${state}=    Execute JavaScript    return responseMap.get("${apiB_tax_toggle}").state;
    ${msg}=    Execute JavaScript    return responseMap.get("${apiB_tax_toggle}").msg;
