...
order009
    ...
    Page Should Contain Element    ${locatorB_order_alerady_send_btn}
    Page Should Contain Element    ${locatorB_order_alerady_mission_btn}
    Page Should Contain Element    ${locatorB_order_not_payment_btn}
    Page Should Contain Element    ${locatorB_order_alerady_cencel_btn}
...
order071
    ...
    click button    添加运单
    sleep    1.5
    page should contain element    ${locatorB_order_four_btn}
...
order080
    ...
    sleep    1
    Wait And Click Element    ${locatorB_order}
    Page Should Contain Element    ${locatorB_order_list_search}

order082
    [Documentation]    图标选中全部栏，订单列表展示全部订单
    [Tags]    P1
    #${time}    getTimeData
    #${order_list_apis}=    Set Variable    http://admin1024.shoplazza.com/api/order/list?start_create_time=${locatorB_start_time}&end_create_time=${locatorB_end_time}&page=0&size=20
    #Go To Order Page
    #Start Ajax Listener
    #Sleep    1.5
    #Wait And Click Element    ${locatorB_order}
    #Sleep    1
    #${dataLength}=    Execute JavaScript    return responseMap.get("${locatorB_order_list_api}").data.list
...
order094
    ...
    Wait And Click Element    ${locatorB_order}
    Wait Until Element Is Visible    ${locatorB_order_list_first_tr}
    Input Text    ${locatorB_order_list_input}    1230000000    #.输入错误订单号
    Wait And Click Element    ${locatorB_order_list_search}    #.搜索按钮
    Sleep    1.5
...
order101
    ...
    Input Text    ${locatorB_order_list_maxprice}    ${max_price}
    Wait And Click Element    ${locatorB_order_list_search}    #.搜索按钮
    ${msg}    Get Value    ${locatorB_order_list_maxprice}
